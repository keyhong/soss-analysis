...
def delete_gu_cctv_mntr_tm(today: str, server: str):
...
def spark_insert_dm_gu_cctv_mntr_tm(arg_date: str):
    ...
    SELECT '{today}' AS stdr_de -- 기준일자
          ...
          , admd_cd
          , cctv_tmzn_cd
          , ROUND(mntr_rate, 3) AS mntr_rate
          , CASE WHEN mntr_rate > 70 AND RANK() OVER (PARTITION BY stdr_de, gu_cd, admd_cd ORDER BY mntr_rate DESC) < 5 THEN 'Y'
            ELSE 'N'
            END AS cnc_mntr_tm_yn
     FROM (
           SELECT stdr_de
                , gu_cd
                , admd_cd
    ...
    df.write.format("jdbc") \
        ...
        .option("dbtable", "SOSS.DM_GU_CCTV_MNTR_TM") \
        .save(mode="append")
